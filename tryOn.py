...
def cvloop(run_event):
    ...
    while run_event.is_set(): 
        ...
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #converting the colour image to gray
        faces = detector(gray, 0)

        for face in faces: 
            (x,y,w,h) = (face.left(), face.top(), face.width(), face.height()) #reading the coordinates in sequence
            ...
            if SPRITES[4]:
                ...
                (x1,y1,w1,h1) = get_face_boundbox(shape, 8)
                apply_sprite(image, image_path,w1+300,x1-205,y1+40, incl,  ontop = False)
